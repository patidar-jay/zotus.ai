...
const pillars = [
  { number: '01', icon: Brain, title: 'AI-First Approach', desc: 'Every monitoring pipeline is powered by purpose-built AI models — not retrofitted rule engines. We train, deploy, and iterate on models tuned to your specific environment.', color: '#6C3CF4' },
  { number: '02', icon: Zap, title: 'Real-Time Processing', desc: 'Sub-200ms detection-to-alert latency. Our edge-optimized inference stack processes every frame in real-time — no batching, no delays, no missed incidents.', color: '#10B981' },
  { number: '03', icon: Users, title: '24/7 Operations Team', desc: 'Dedicated operations engineers monitor system health around the clock. Escalation protocols, on-call rotations, and proactive maintenance — all included.', color: '#F59E0B' },
  { number: '04', icon: Rocket, title: 'Rapid Deployment', desc: 'Go from contract to production in under two weeks. Pre-built deployment templates, hardware-agnostic architecture, and white-glove onboarding for every client.', color: '#EC4899' },
];

const stats = [
  { value: '99.2%', label: 'Detection Accuracy', icon: Shield },
  { value: '<200ms', label: 'Alert Latency', icon: Zap },
...
export default function WhyChooseUsSection() {
  return (
    <>
      {/* Why Us — 4 numbered pillars with descriptions */}
        ...
        <Container className="relative z-10">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-14 md:mb-20">
            ...
            <p className="mx-auto mt-4 max-w-2xl text-base md:text-lg" style={{ color: 'var(--color-text-secondary)' }}>
              Four pillars. One commitment. We handle the complexity so your team can focus on what matters.
            </p>
          </motion.div>
      ...
      {/* Deep-Dive Section — like TBrain's Expert OS feature section */}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
              ...
              <h2 className="text-3xl md:text-5xl font-semibold leading-tight mb-6" style={{ fontFamily: 'var(--font-heading)' }}>
                ...
                <span style={{ background: 'linear-gradient(120deg, #A78BFA, #10B981)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>built for scale.</span>
              </h2>
              <p className="text-base md:text-lg leading-relaxed mb-8" style={{ color: 'var(--color-text-secondary)' }}>
                Purpose-built infrastructure for organizations that need reliable, real-time monitoring at scale — without building it themselves.
              </p>
                  ...
                  {[0, 1, 2, 3].map(i => (
                    <div key={i} className="rounded-xl relative overflow-hidden" style={{ background: 'rgba(108,60,244,0.04)', border: '1px solid rgba(108,60,244,0.08)' }}>
                      <div className="absolute top-2 left-2 flex items-center gap-1.5">
                        <span className="h-1.5 w-1.5 rounded-full" style={{ background: '#10B981', animation: `pulse ${1.5 + i * 0.3}s ease infinite` }} />
